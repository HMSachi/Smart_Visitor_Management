  ...
  <div className="p-4 border border-mas-border bg-mas-dark hover:border-mas-red transition-all group">
    <div className="flex justify-between items-start mb-2">
      <div className="flex items-center gap-2">
        ...
        <span className="text-xs font-bold uppercase tracking-tight text-white">{type}</span>
      </div>
      <span className="text-[10px] text-mas-text-dim uppercase font-bold">{time}</span>
    </div>
    <div className="flex items-center gap-2 mb-3">
      <MapPin size={12} className="text-mas-text-dim" />
      <span className="text-[10px] text-mas-text-dim uppercase tracking-widest">{location}</span>
    </div>
    <div className="flex gap-2">
      <button className="flex-1 bg-mas-gray text-[9px] uppercase font-bold tracking-[0.2em] py-1.5 border border-mas-border hover:border-mas-red hover:text-white transition-all">
        Investigate
      </button>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div className="lg:col-span-2 mas-panel min-h-[400px] relative overflow-hidden group border-white/[0.05]">
        ...
        <div className="absolute top-6 left-6 z-20">
          <h2 className="text-sm font-bold uppercase tracking-[0.2em] text-white">Zone Tracking Map</h2>
          <p className="text-[10px] text-mas-text-dim uppercase tracking-widest mt-1">Real-time visitor location monitoring</p>
        </div>
        ...
        <div className="absolute inset-0 flex items-center justify-center bg-[#0d0d0e]">
          ...
          <p className="z-20 text-[10px] font-black text-mas-text-dim uppercase tracking-[0.5em] animate-pulse">Scanning live zones...</p>
        </div>
        ...
        <div className="absolute bottom-6 right-6 z-20 flex gap-2">
          ...
          <button className="mas-button-primary py-1 px-4 text-[10px]">Expand view</button>
        </div>
      </div>
      ...
      <div className="mas-panel flex flex-col h-full border-white/[0.05]">
        <div className="p-6 border-b border-mas-border flex justify-between items-center bg-mas-dark/50">
          <h2 className="text-sm font-black uppercase tracking-[0.2em] text-white">Live Alerts</h2>
        ...
        <div className="p-4 border-t border-mas-border bg-mas-dark/30 text-center">
          <button className="text-[10px] uppercase font-black tracking-widest text-mas-text-dim hover:text-white transition-all underline">
