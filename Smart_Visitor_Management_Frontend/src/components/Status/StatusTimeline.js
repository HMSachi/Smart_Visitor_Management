        ...
        <div className="space-y-12 relative">
            ...
            {stages.map((stage, idx) => (
                <div key={stage.id} className="relative pl-12 group">
                    {/* Node */}
                    <div className={`absolute left-0 top-1 w-[24px] h-[24px] border-2 transition-all duration-500 ${
                        stage.status === 'completed' ? 'bg-mas-red border-mas-red' :
                        stage.status === 'active' ? 'bg-charcoal-900 border-mas-red animate-pulse shadow-[0_0_15px_#C8102E]' :
                        'bg-charcoal-900 border-white/20 group-hover:border-white/40 shadow-sm'
                    }`}>
                    ...
                    <div>
                        <h4 className={`text-[10px] uppercase tracking-[0.3em] font-black mb-1 ${
                            stage.status === 'upcoming' ? 'text-gray-400' : 'text-white'
                        }`}>
                            {stage.label}
                        </h4>
                        <p className={`text-[9px] uppercase tracking-widest font-mono ${
                            stage.status === 'active' ? 'text-mas-red' : 'text-charcoal-400'
                        } ${stage.status === 'upcoming' ? 'text-gray-500' : 'text-gray-300'}`}>
                            {stage.time}
                        </p>
                    </div>
                </div>
