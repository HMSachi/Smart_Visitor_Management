                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between mb-24 gap-12">
                    <div className="max-w-xl">
                        <div className="flex items-center space-x-4 mb-4">
                            ...
                            <span className="text-[10px] uppercase tracking-[0.4em] font-bold text-mas-red">Security First</span>
                        </div>
                        <h2 className="text-4xl md:text-5xl font-display font-black tracking-tight text-white uppercase mb-6">
                ...
                <div className="grid grid-cols-1 gap-8 md:grid-cols-3">
                    {categories.map((category, index) => (
                        <div key={index} className="glass-panel p-12 border-mas-red/10 hover:border-mas-red/40 transition-all duration-500 overflow-hidden relative group">
                            ...
                            <span className="text-mas-red text-[11px] font-black uppercase tracking-[0.3em] mb-8 inline-block">{category.name}</span>
                            <ul className="space-y-6">
                                {category.items.map((item, idx) => (
                                    <li key={idx} className="flex items-start text-white text-sm font-light">
                ...
                <div className="mt-24 p-1 bg-gradient-to-r from-mas-red/40 via-mas-red/10 to-transparent">
                    <div className="bg-charcoal-900 p-12 md:p-16 flex flex-col md:flex-row items-center justify-between text-white relative">
                        <div className="relative z-10 text-center md:text-left mb-12 md:mb-0 max-w-lg">
                            <h3 className="text-3xl font-display font-black uppercase mb-4 tracking-tight">Access Clearance Required?</h3>
                            <p className="text-gray-200 font-light">Initiate your digital clearance request for secure entry to MAS Holdings facilities.</p>
                        </div>
                        <Link to="/request-step-1" className="relative z-10 px-16 py-6 bg-mas-red text-white text-[12px] font-black uppercase tracking-[0.3em] hover:bg-[#A60D26] shadow-[0_0_30px_rgba(200,16,46,0.2)] transition-all transform active:scale-95 text-center">
                            Request Visit
                        </Link>
                    </div>
                </div>
