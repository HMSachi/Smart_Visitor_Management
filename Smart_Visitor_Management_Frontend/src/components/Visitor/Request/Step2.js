...
const Step2Main = () => {
    ...
    const step1Summary = {
        ...
        visitDate: visitorState.visitDate,
        purpose: visitorState.purpose,
        visitorCount: visitorState.visitorCount
    };
    ...
    const handleFileUpload = (e) => {
        ...
        if (file) {
            dispatch(updateField({ name: 'uploadedFile', value: file.name }));
    ...
    const removeUploadedFile = () => {
        dispatch(updateField({ name: 'uploadedFile', value: null }));
        dispatch(setUploadProgress(0));
    };
    ...
    if (status === 'review') {
        return (
            <div className="min-h-screen bg-charcoal-900 flex items-center justify-center p-6 text-white text-center">
                <div className="max-w-xl w-full glass-panel p-16 border-mas-red/20 shadow-[0_0_50px_rgba(200,16,46,0.1)] transition-all transform scale-100 opacity-100">
