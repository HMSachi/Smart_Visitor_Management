    ...
    return (
        <section className="animate-fade-in group">
            <div className="flex items-center space-x-6 mb-12">
                ...
                <h2 className="text-white uppercase">Visitor Overview</h2>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-10 border-t border-white/5 pt-12">
                {fields.map((field) => (
                    <div key={field.name} className="relative group/field">
                        <label className="uppercase text-gray-300 mb-3 block group-focus-within/field:text-mas-red transition-colors">{field.label}</label>
                        <input 
                            type={field.type}
                            name={field.name}
