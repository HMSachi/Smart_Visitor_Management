...
const NotificationPanel = () => {
    ...
    return (
        <section className="bg-white/[0.02] border border-white/5 p-8 h-full">
            <h3 className="uppercase text-white mb-10 pb-4 border-b border-white/5 flex justify-between items-center">
                Security Log
                ...
                {alerts.map((alert) => (
                    <div key={alert.id} className="group border-l border-white/10 pl-6 py-2 hover:border-mas-red transition-all cursor-crosshair">
                        <div className="flex justify-between items-start mb-2">
                            <span className="uppercase text-mas-red">{alert.type}</span>
            ...
            <div className="mt-20 pt-8 border-t border-white/5">
                <div className="p-4 bg-mas-red/5 border border-mas-red/10">
                    <p className="text-gray-400 uppercase">
                        Security Notice: All interactions within this node are logged and encrypted using <span className="text-white">AES-256</span>.
                    </p>
                </div>
            </div>
        </section>
