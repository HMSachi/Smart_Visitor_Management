        ...
        <div className={`glass-panel p-12 border-l-4 ${current.border.replace('border-', 'border-l-')} relative overflow-hidden`}>
            <div className="flex flex-col md:flex-row items-center justify-between text-center md:text-left">
                <div className="mb-8 md:mb-0">
                    <span className={`uppercase ${current.color} mb-4 block`}>Current Protocol State</span>
                    <h2 className="text-white uppercase flex items-center justify-center md:justify-start">
                        <span className="mr-6">{current.icon}</span> {current.label}
                    </h2>
                </div>
                <div className="bg-white/5 border border-white/5 px-10 py-6">
                    <span className="text-gray-400 uppercase block mb-2">Estimated Completion</span>
                    <span className="text-white">~ 14 MINUTES</span>
                </div>
            </div>
