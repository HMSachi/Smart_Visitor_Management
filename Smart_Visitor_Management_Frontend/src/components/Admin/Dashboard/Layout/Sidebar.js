...
const SidebarItem = ({ icon: Icon, label, active, onClick }) => (
  ...
  >
    ...
    <span className={`uppercase transition-all ${active ? 'text-white' : 'text-mas-text-dim group-hover:text-white'}`}>
      {label}
    </span>
  </div>
);
    ...
    <div className="w-72 h-screen fixed left-0 top-0 bg-mas-dark border-r border-mas-border flex flex-col z-50">
      <div className="p-8 border-b border-mas-border">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-white">MAS</h1>
            <p className="text-mas-text-dim uppercase">Visitor Management</p>
          </div>
        </div>
      </div>
      ...
      <div className="p-6 border-t border-mas-border">
        <div className="flex items-center gap-3 px-2">
          <div className="w-8 h-8 bg-mas-gray flex items-center justify-center border border-mas-border">
            <span className="text-mas-red">JD</span>
          </div>
          <div>
            <p className="text-white">Samith</p>
            <p className="text-mas-text-dim uppercase">Senior Administrator</p>
          </div>
        </div>
      </div>
    </div>
