...
const StatusBadge = ({ status }) => {
  ...
  return (
    <span className={`px-2 py-1 uppercase border ${styles[status] || styles['Pending']}`}>
      {status}
    </span>
  );
};
    ...
    <div className="bg-[#0F0F10] border border-white/5 overflow-hidden">
      ...
      <div className="px-8 pt-4 pb-6 border-b border-white/5 relative bg-[#0F0F10]">
        <div className="flex justify-between items-end mb-4">
          ...
          <p className="text-mas-text-dim uppercase">Authorization Registry</p>
        </div>
      ...
      <div className="p-6 border-b border-white/5 bg-[#121212] flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="uppercase text-white">Visitor Authorization</h2>
            <p className="text-mas-text-dim uppercase mt-2">Clearance Protocol Hub</p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-4 w-full md:w-auto">
          <div className="relative flex-1 md:w-80">
            <input 
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-[#0F0F10] border border-white/10 uppercase text-white placeholder:text-white/20 focus:border-mas-red outline-none transition-all"
              value={searchTerm}
          ...
          <select 
            ...
            className="px-5 py-2.5 bg-[#0F0F10] border border-white/10 uppercase text-mas-text-dim focus:text-white focus:border-mas-red transition-all cursor-pointer outline-none appearance-none"
          >
          ...
          <button className="flex items-center gap-3 px-5 py-2.5 border border-white/10 uppercase text-mas-text-dim hover:text-white hover:border-mas-red transition-all group">
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-[#121212] border-b border-white/5">
              <th className="px-8 py-4 uppercase text-mas-text-dim text-center w-16">Unit</th>
              <th className="px-8 py-4 uppercase text-mas-text-dim">Main Identity</th>
              <th className="px-8 py-4 uppercase text-mas-text-dim">Reference / Contact</th>
              <th className="px-8 py-4 uppercase text-mas-text-dim">Schedule Protocol</th>
              <th className="px-8 py-4 uppercase text-mas-text-dim text-center">Authorization</th>
              <th className="px-8 py-4 uppercase text-mas-text-dim text-right pr-12 text-mas-red">Control</th>
            </tr>
          </thead>
              ...
              <React.Fragment key={visitor.batchId}>
                <tr className={`group transition-all ${expandedBatches.includes(visitor.batchId) ? 'bg-[#121212]' : 'hover:bg-white/[0.01]'}`}>
                  <td className="px-8 py-4 text-center">
                    ...
                    >
                      {expandedBatches.includes(visitor.batchId) ? <ChevronUp size={10} /> : <div className="">{visitor.members.length + 1}</div>}
                    </button>
                  </td>
                  <td className="px-8 py-4">
                    <div className="flex items-center gap-5">
                      <div className="w-9 h-9 bg-black border border-white/10 flex items-center justify-center text-mas-red group-hover:border-mas-red transition-all shrink-0">
                        {visitor.name.split(' ').map(n => n[0]).join('')}
                      </div>
                      <div>
                        <p className="text-white uppercase mb-1">{visitor.name}</p>
                        <p className="text-mas-red/60 uppercase">Lead Personnel</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-8 py-4">
                    <p className="text-white uppercase mb-1">{visitor.batchId}</p>
                    <p className="text-mas-text-dim uppercase">{visitor.contactPerson}</p>
                  </td>
                  <td className="px-8 py-4">
                    <p className="text-white uppercase mb-1">{visitor.date} @ {visitor.timeIn}</p>
                    <p className="text-mas-text-dim uppercase line-clamp-1">{visitor.areas.join(' | ')}</p>
                  </td>
                  <td className="px-8 py-4 text-center">
