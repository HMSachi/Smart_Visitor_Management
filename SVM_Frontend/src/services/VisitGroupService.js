...
const GetAllVisitGroup = async () => {
    let config = {
        method: 'get',
        url: 'https://visitormanagement.dockyardsoftware.com/VisitGroup/GetAllVisitGroup'
    };
...
const GetVisitGroupById = async (id) => {
    let config = {
        method: 'get',
        url: `https://visitormanagement.dockyardsoftware.com/VisitGroup/GetVisitGroupById/?VVG_id=${encodeURIComponent(id)}`
    };
    return axios.request(config).then((response) => response);
}
...
const UpdateVisitGroupStatus = async (id, status) => {
    let config = {
        method: 'post',
        url: `https://visitormanagement.dockyardsoftware.com/VisitGroup/GetActiveVisitGroup/?VVG_id=${encodeURIComponent(id)}&VVG_Status=${encodeURIComponent(status)}`,
        data: ''
    };
    return axios.request(config).then((response) => response);
}
