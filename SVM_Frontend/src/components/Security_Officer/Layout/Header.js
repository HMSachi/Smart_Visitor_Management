...
const Header = ({ title }) => {
    ...
    return (
        <header className="h-20 md:h-24 border-b border-white/10 bg-mas-black/95 backdrop-blur flex items-center justify-between px-6 md:px-12 sticky top-0 z-40">
            <div className="flex items-center gap-4 md:gap-6 truncate mr-4">
                <button 
                    onClick={() => navigate(-1)} 
            ...
            <div className="flex items-center gap-6 md:gap-12 flex-shrink-0">
                <div className="hidden lg:flex items-center gap-4 py-2 px-4 border border-white/5 bg-white/[0.02]">
                ...
                <div className="flex items-center gap-3">
                    <div className="hidden md:flex flex-col items-end">
                        <span className="text-mas-text-dim uppercase text-[10px]">Network Node</span>
                        <div className="flex items-center gap-2">
                             <div className="w-1.5 h-1.5 bg-green-500 rounded-full animate-pulse shadow-[0_0_5px_#22c55e]"></div>
                             <span className="text-white uppercase text-[10px]">PRIMARY ENCRYPTED</span>
                        </div>
                    </div>
