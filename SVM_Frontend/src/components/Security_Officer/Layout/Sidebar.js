...
const menuItems = [
  { id: 'dashboard', label: 'Dashboard', icon: ShieldCheck, path: '/Security_Officer/dashboard' },
  { id: 'scanner', label: 'Scan QR Code', icon: QrCode, path: '/Security_Officer/scanner' },
...
const SidebarItem = ({ icon: Icon, label, active, onClick, collapsed }) => (
  ...
  >
    ...
    {!collapsed && (
      <span className="text-[13.5px] font-medium whitespace-nowrap overflow-hidden text-ellipsis">{label}</span>
    )}
    {active && !collapsed && (
...
const SidebarContent = ({ isCollapsed, currentPath, onNavigate, onLogout, user }) => {
  ...
  const displayEmail = user?.ResultSet?.[0]?.VA_Email || user?.ResultSet?.[0]?.VS_Email || '';
      ...
      <div className={`flex items-center gap-3 px-4 py-5 border-b border-[var(--color-border-soft)] ${isCollapsed ? 'justify-center' : ''}`}>
        ...
        {!isCollapsed && (
          <div className="min-w-0 animate-fade-in">
            <p className="text-[var(--color-text-primary)] text-[13px] font-bold tracking-tight leading-tight">Security Portal</p>
        ...
        <div className={`flex items-center gap-3 px-2 ${isCollapsed ? 'justify-center' : ''}`}>
          ...
          {!isCollapsed && (
            <div className="min-w-0 animate-fade-in">
              ...
              <p className="text-[var(--color-text-dim)] text-[11px] truncate">{displayEmail || 'Security Officer'}</p>
            </div>
          )}
        </div>
      </div>
