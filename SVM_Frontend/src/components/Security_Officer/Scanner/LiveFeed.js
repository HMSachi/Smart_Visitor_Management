...
import { QrCode, Zap, RefreshCw, User, Clipboard, ShieldCheck, AlertTriangle, ArrowRight } from 'lucide-react';
...
const LiveFeed = () => {
    ...
    const handleVerification = async (data) => {
        ...
        try {
            ...
            const details = result?.ResultSet?.[0] || result?.[0] || result;

            if (details) {
                setPassDetails(details);
                setScanStatus('details');
                setScanMessage('Access protocol verified and decrypted.');
            } else {
                throw new Error("Invalid Credentials");
            }
        } catch (err) {
            setScanStatus('error');
            setScanMessage('Verification Failed: The scanned credential is not recognized by the central node.');
        } finally {
            setIsLoading(false);
        }
    };
        ...
        <div className="max-w-4xl w-full space-y-6 md:space-y-12 relative z-10 mx-auto px-4">
            <div className="text-center space-y-4">
                <div className="flex items-center justify-center gap-4 mb-2">
                    ...
                    <span className="text-primary uppercase tracking-[0.3em] text-[10px] font-bold">Node-04 // Security Perimeter</span>
                </div>
                <h1 className="uppercase text-3xl font-black tracking-tight italic">Tactical Scanner</h1>
            </div>
                    ...
                    >
                            ...
                            <div className="w-full h-full bg-[#0A0A0B] relative overflow-hidden flex items-center justify-center rounded-[24px]">
                                ...
                                {scanStatus === 'success' && (
                                    <div className="absolute inset-0 z-30 bg-green-500/20 flex flex-col items-center justify-center backdrop-blur-md">
                                        ...
                                        <p className="mt-8 text-green-500 uppercase font-black tracking-[0.3em]">Payload Intercepted</p>
                                    </div>
                                )}
                                ...
                                {scanStatus === 'error' && (
                                    <div className="absolute inset-0 z-30 bg-primary/20 flex flex-col items-center justify-center backdrop-blur-md px-10 text-center">
                                        ...
                                        <p className="mt-8 text-primary uppercase font-black tracking-[0.3em]">Perimeter Breach</p>
                                        <p className="mt-4 text-white/70 text-[10px] uppercase tracking-widest">{scanMessage}</p>
                                    </div>
                                )}
                        ...
                        <div className="flex flex-col items-center gap-8">
                            ...
                            >
                                ...
                                {scanStatus === 'scanning' ? 'Scanning Feed...' : 'Initiate Scan'}
                            </button>

                            <p className="text-gray-400 uppercase text-[10px] tracking-[0.3em] text-center max-w-sm leading-relaxed">{scanMessage}</p>
                        </div>
                    </motion.div>
                ) : (
                    <motion.div
                    ...
                    >
                        ...
                        <div className="p-10 border-b border-white/5 flex items-center justify-between bg-white/[0.01]">
                            <div className="flex items-center gap-6">
                                ...
                                <div>
                                    <h2 className="text-white text-2xl font-black uppercase tracking-tight italic">Clearance Verified</h2>
                                    <p className="text-green-500 font-bold uppercase tracking-[0.3em] text-[10px] mt-1">Status: Personnel Active</p>
                                </div>
                            </div>
                            <div className="text-right">
                                <span className="text-gray-500 uppercase text-[10px] font-bold tracking-[0.3em]">Pass Node</span>
                                <p className="text-white font-mono text-lg font-bold">#{passDetails?.VGP_Pass_id}</p>
                            </div>
                        </div>
                        ...
                        <div className="p-10 grid grid-cols-2 gap-10">
                            <div className="space-y-3">
                                ...
                                <p className="text-white text-lg font-bold uppercase tracking-widest bg-white/5 p-4 rounded-2xl border border-white/5 shadow-inner">
                                    Visitor #{passDetails?.VGP_Visitor_id || 'UNKNOWN'}
                                </p>
                            </div>
                            <div className="space-y-3">
                                <label className="flex items-center gap-2 text-gray-500 uppercase text-[10px] font-bold tracking-[0.3em]">
                                    <Clipboard size={12} className="text-primary" /> Request Protocol
                                </label>
                                <p className="text-white text-lg font-bold uppercase tracking-widest bg-white/5 p-4 rounded-2xl border border-white/5 shadow-inner">
                                    REG-NODE-{passDetails?.VGP_Request_id || '000'}
                                </p>
                            </div>
                            <div className="col-span-2 space-y-3 pt-4">
                                <label className="flex items-center gap-2 text-gray-500 uppercase text-[10px] font-bold tracking-[0.3em]">
                                    <Zap size={12} className="text-primary" /> Authorization Insights
                                </label>
                                <div className="bg-green-500/5 border border-green-500/10 rounded-3xl p-6 flex items-center justify-between">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <span className="text-white/90 text-[11px] uppercase tracking-widest font-medium italic">Credential integrity confirmed via Cloud-Link</span>
                                    </div>
            ...
            <div className="flex items-center justify-center gap-8 py-8 border-t border-white/5 opacity-50">
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-gray-500 uppercase text-[9px] font-bold tracking-widest">Biometric: Active</span>
                </div>
                <div className="flex items-center gap-2">
                    <div className="w-1.5 h-1.5 bg-primary rounded-full"></div>
                    <span className="text-gray-500 uppercase text-[9px] font-bold tracking-widest">Log: Syncing</span>
                </div>
                <div className="flex items-center gap-2">
                    <div className="w-1.5 h-1.5 bg-primary rounded-full"></div>
                    <span className="text-gray-500 uppercase text-[9px] font-bold tracking-widest">Node: {navigator.onLine ? 'ONLINE' : 'OFFLINE'}</span>
                </div>
            </div>
        </div>
    );
