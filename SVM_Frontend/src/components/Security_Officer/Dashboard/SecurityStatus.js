...
const SecurityStatus = () => {
  ...
  const healthStatus =
    systemHealth >= 95 ? "Optimal" : systemHealth >= 80 ? "Good" : "Warning";
  const healthColor =
    systemHealth >= 95
      ? "text-green-500"
      : systemHealth >= 80
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-6">
        <div>
          ...
          <p className="text-[var(--color-text-secondary)] text-xs opacity-75 mt-1 uppercase tracking-[0.2em]">
            Infrastructure & gateway health
          </p>
        </div>
        <div
          className={`px-4 py-2 rounded-lg bg-${healthColor === "text-green-500" ? "green" : healthColor === "text-yellow-500" ? "yellow" : "primary"}/10 border border-${healthColor === "text-green-500" ? "green" : healthColor === "text-yellow-500" ? "yellow" : "primary"}/20`}
        >
          <span
            className={`text-sm font-bold uppercase tracking-wide ${healthColor}`}
          >
            {healthStatus}
          </span>
        </div>
