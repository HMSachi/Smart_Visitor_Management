...
const StatusTimeline = ({ currentStage }) => {
  ...
  const stagesFixed = [
    ...
    },
    {
      ...
      time: "Under MAS Review",
      icon: Clock,
    },
    {
      id: "step2_pending",
      label: "Detailed Clearance",
      ...
      {stagesFixed.map((stage, idx) => {
        ...
        return (
          ...
          >
            {/* Node Architecture */}
            <div
              className={`absolute left-0 top-0 w-8 h-8 rounded-full border-2 flex items-center justify-center transition-all duration-700 z-10 ${
                status === "completed"
            ...
            <div className="flex flex-col gap-1">
              <h4
                className={`text-[11px] font-normal uppercase tracking-[0.2em] transition-colors ${
                  status === "upcoming"
                    ? "text-gray-400"
                    : status === "active"
                      ? "text-mas-red underline underline-offset-4 decoration-mas-red/20"
                      : "text-white"
                }`}
              >
                {stage.label}
              ...
              <div className="flex items-center gap-3">
                ...
                {status === "completed" && (
                  <span className="text-[8px] font-medium text-mas-red/80 uppercase tracking-tighter">
                    Verified
                  </span>
                )}
              </div>
            ...
            <div className="mt-4 opacity-0 group-hover:opacity-100 transition-all duration-300 max-h-0 group-hover:max-h-20 overflow-hidden pointer-events-none hidden md:block">
              <div className="bg-white/[0.03] border border-white/10 p-4 rounded-xl backdrop-blur-md inline-block">
                <p className="text-[9px] text-gray-400 uppercase font-medium tracking-widest mb-1">
                  Target Module
                </p>
                <p className="text-white text-[10px] font-normal">
                  Facility Clearance Section {idx + 1}
                </p>
              </div>
            </div>
          </motion.div>
