...
const Step2Main = () => {
    ...
    if (status === 'step2_pending') {
        return (
            <div className="min-h-[80vh] flex items-center justify-center p-6 bg-mas-dark-900 border-t border-white/5">
                <div className="max-w-xl w-full bg-white/[0.02] border border-white/5 p-12 text-center rounded-2xl shadow-xl">
                    <div className="w-12 h-12 bg-mas-red/10 border border-mas-red/20 rounded-xl mx-auto mb-8 flex items-center justify-center text-mas-red">
                        <ShieldCheck size={24} />
                    </div>
    ...
    return (
        <div className="max-w-4xl mx-auto px-6 pt-2 pb-12 text-white bg-mas-dark-900 border-t border-white/5">
            {/* Phase Header */}
                ...
                <div className="space-y-10 pt-6">
                    <div 
                        className={`p-6 bg-white/[0.01] border rounded-xl flex items-start gap-4 cursor-pointer transition-all ${
                            isConfirmed ? 'border-mas-red/50 bg-mas-red/5' : 'border-white/5 hover:border-white/10'
                        }`}
                        onClick={() => setIsConfirmed(!isConfirmed)}
                    >
                        <div className={`mt-0.5 min-w-[20px] h-5 flex items-center justify-center rounded border transition-all ${
                            isConfirmed ? 'bg-mas-red border-mas-red' : 'border-white/20'
                        }`}>
