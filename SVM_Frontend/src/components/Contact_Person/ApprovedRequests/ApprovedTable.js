...
<table className="w-full text-left border-separate border-spacing-y-4 sm:border-spacing-y-0 sm:border-collapse min-w-0 sm:min-w-[700px] block sm:table">
                <thead className="hidden sm:table-header-group font-normal text-[12px]">
                    <tr className={`border-b text-[12px] font-normal uppercase tracking-[0.3em] transition-colors ${
                        ...
                        <th className="px-8 py-2 w-24 font-normal text-[12px]">ID</th>
                        <th className="px-8 py-2 text-left font-normal text-[12px]">Visitor</th>
                        <th className="px-8 py-2 text-center font-normal text-[12px]">Date</th>
                        <th className="px-8 py-2 text-center font-normal text-[12px]">Status</th>
                ...
                <tbody className="block sm:table-row-group">
                    {requests.map((req) => (
                        ...
                        >
                            <td className={`block sm:table-cell px-2 sm:px-8 py-1 border-b sm:border-none last:border-none ${isLight ? "border-gray-50" : "border-white/5"}`}>
                             ...
                             <td className={`block sm:table-cell px-2 sm:px-8 py-1 border-b sm:border-none last:border-none uppercase tracking-wide ${isLight ? "border-gray-50" : "border-white/5"}`}>
                                ...
                                <p className={`text-[12px] font-normal ${isLight ? "text-[#1A1A1A]" : "text-white"}`}>{req.name}</p>
                            </td>
                            <td className={`block sm:table-cell px-2 sm:px-8 py-1 border-b sm:border-none last:border-none text-center ${isLight ? "border-gray-50" : "border-white/5"}`}>
                                ...
                                <div className="flex flex-col items-center justify-center gap-1">
                                    <div className={`text-[12px] font-normal ${isLight ? "text-[#1A1A1A]" : "text-white/80"}`}>
                                        {req.date}
                                    </div>
                                </div>
                            </td>
                            <td className={`block sm:table-cell px-2 sm:px-8 py-1 text-left sm:text-center border-b sm:border-none last:border-none ${isLight ? "border-gray-50" : "border-white/5"}`}>
                                <span className="text-[12px] font-normal tracking-[0.2em] text-primary/60 uppercase block sm:hidden mb-3 text-left">Status</span>
                                <div className={`inline-flex flex-col md:flex-row items-center gap-4 md:gap-2 px-2 py-0.5 rounded-md border text-[9px] font-normal uppercase tracking-widest ${getAdminStatusStyle(req.adminStatus)}`}>
                                    {req.adminStatus}
                                </div>
                            </td>
                            <td className="block sm:table-cell px-2 sm:px-8 py-1 text-left sm:text-right font-normal text-[12px]">
                                ...
                                <button
                                    ...
                                    className={`inline-flex flex-col md:flex-row items-center gap-4 md:gap-2 text-[10px] font-normal uppercase tracking-widest transition-colors group ${isLight ? "text-gray-500 hover:text-primary" : "text-white/40 hover:text-primary"}`}
                                >
                                    View
                    ...
                    {requests.length === 0 && (
                        <tr>
                            <td colSpan="5" className="px-8 py-20 text-center text-gray-300 uppercase tracking-[0.3em] font-normal opacity-30 text-[12px]">
