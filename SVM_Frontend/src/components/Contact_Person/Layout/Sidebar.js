...
const menuItems = [
  ...
  { id: 'inbox', label: 'Approval Inbox', icon: Inbox, path: '/contact_person/requests-inbox' },
  { id: 'all-visitors', label: 'All Visitors', icon: Users, path: '/contact_person/all-visitors' },
];
...
const SidebarContent = ({ isCollapsed, currentPath, onNavigate, onLogout, user }) => {
  const displayName = user?.ResultSet?.[0]?.VCP_Name || user?.ResultSet?.[0]?.VA_Name || 'Host';
  const displayEmail = user?.ResultSet?.[0]?.VCP_Email || user?.ResultSet?.[0]?.VA_Email || '';
  const initials = displayName.split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase() || 'HO';
  ...
  return (
    <Box className="h-full flex flex-col" style={{ background: 'var(--color-bg-paper)', borderRight: '1px solid var(--color-border-soft)' }}>
      ...
      <div className={`flex items-center gap-3 px-4 py-5 border-b border-[var(--color-border-soft)] ${isCollapsed ? 'justify-center' : ''}`}>
        <img src="/logo_mas.png" alt="MAS Logo" className="h-7 w-auto shrink-0 transition-all duration-300" />
        {!isCollapsed && (
          <div className="min-w-0 animate-fade-in">
            <p className="text-[var(--color-text-primary)] text-[13px] font-bold tracking-tight leading-tight">Host Portal</p>
        ...
        <div className={`flex items-center gap-3 px-2 ${isCollapsed ? 'justify-center' : ''}`}>
          ...
          {!isCollapsed && (
            <div className="min-w-0 animate-fade-in">
              ...
              <p className="text-[var(--color-text-dim)] text-[11px] truncate">{displayEmail || 'Host / Contact Person'}</p>
            </div>
          )}
        </div>
      </div>
...
const Sidebar = () => {
  ...
  const contentProps = { isCollapsed: false, currentPath: location.pathname, onNavigate: handleNavigate, onLogout: handleLogout, user };
  ...
  return (
    <aside className="flex-none relative h-screen transition-[width] duration-500 ease-in-out z-40" style={{ width: isCollapsed ? '72px' : '256px' }}>
