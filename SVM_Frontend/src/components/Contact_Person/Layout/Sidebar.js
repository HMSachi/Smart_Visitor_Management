  ...
  return (
    <Box className="h-full flex flex-col pt-10 p-4 bg-mas-dark/95 border-r border-white/10">
      ...
      <div className={`mb-12 flex items-center ${isCollapsed ? 'justify-center' : 'px-4 gap-3'}`}>
         ...
         {!isCollapsed && <span className="text-white font-medium tracking-tighter text-sm flex-none uppercase animate-fade-in">Contact <span className="text-primary">Portal</span></span>}
      </div>
        ...
        <div className={`flex items-center gap-4 ${isCollapsed ? 'justify-center' : 'px-2'}`}>
            ...
            {!isCollapsed && (
                <div className="animate-fade-in overflow-hidden">
                    <p className="text-white text-[13px] font-medium uppercase tracking-wider truncate">Sachi</p>
                    <p className="text-gray-500 text-[14px] uppercase tracking-widest truncate">Contact Person</p>
                </div>
            )}
        </div>
      </div>
