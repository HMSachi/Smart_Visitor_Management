...
const Header = ({ title }) => {
    ...
    return (
        <header className="h-24 pt-6 bg-[var(--color-bg-default)]/90 backdrop-blur-xl sticky top-0 z-40 px-4 md:px-10 flex items-center justify-between border-b border-[var(--color-border-soft)] shadow-2xl">
            <div className="flex items-center gap-4 md:gap-10 flex-1 truncate">
                {isMobile ? (
                    <button
                        onClick={() => dispatch(toggleMobileMenu())}
                        className="p-2.5 text-primary transition-all bg-[var(--color-surface-1)] border border-[var(--color-border-soft)] rounded-xl hover:bg-primary/10 active:scale-95 shadow-lg flex-shrink-0"
                    >
                    ...
                    <button
                        onClick={() => navigate(-1)}
                        className="p-2 rounded-xl text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] transition-all bg-[var(--color-surface-1)] border border-[var(--color-border-soft)] hover:border-primary/30 group flex-shrink-0"
                        title="Go Back"
                    >
                ...
                <div className="flex flex-col">
                    ...
                    <h2 className="text-sm md:text-base font-bold tracking-tight text-[var(--color-text-primary)] uppercase truncate">{title}</h2>
                </div>

            </div>

            <div className="flex items-center gap-8">
                <div className="hidden md:flex items-center gap-6 pr-8 border-r border-[var(--color-border-soft)]">
