                ...
                <tbody className="block sm:table-row-group">
                    {history.map((item) => (
                        <React.Fragment key={item.id}>
                            <tr
                                className={`group transition-all cursor-pointer block sm:table-row bg-[#161618] sm:bg-transparent border border-white/5 sm:border-none rounded-3xl sm:rounded-tl-none sm:rounded-tr-none mb-0 sm:mb-0 p-6 sm:p-0 shadow-2xl sm:shadow-none ${expandedRow === item.id ? 'bg-mas-red/[0.03]' : 'hover:bg-white/[0.01]'}`}
