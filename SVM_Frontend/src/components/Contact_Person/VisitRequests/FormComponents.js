  ...
  <div className="flex items-start gap-3 mb-4">
    ...
    <div className="flex flex-col gap-0.5">
      <div className="flex items-center gap-2">
        ...
        <h3 className="text-[13px] font-medium capitalize tracking-[0.15em] text-[#0A1D37]">
          {title}
        </h3>
      </div>
      {subtitle && (
...
export const InputField = ({ label, name, type = "text", placeholder, value, onChange, error, icon: Icon, disabled = false, list }) => (
  <div className="space-y-1.5">
    ...
    <input
      ...
      disabled={disabled}
      list={list}
      className={`w-full bg-white border rounded-lg px-3 py-1.5 text-[12px] font-medium transition-all placeholder:text-gray-300 focus:outline-none focus:ring-2 focus:ring-primary/5 disabled:bg-gray-50 disabled:text-gray-500 disabled:cursor-not-allowed ${
        error ? "border-red-500" : "border-gray-200 focus:border-primary/50"
      }`}
    />
    {error && <p className="text-[8px] text-red-500 font-bold px-0.5 capitalize">{error}</p>}
  </div>
);
