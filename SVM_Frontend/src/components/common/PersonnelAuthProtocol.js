...
const SectionHeader = ({ title, icon: Icon, isLight }) => (
  <div className="flex items-center gap-5 mb-10 group/header">
    ...
    <div className={`flex-1 h-[1px] ml-4 group-hover/header:bg-primary/20 transition-all ${isLight ? "bg-gray-100" : "bg-white/5"}`}></div>
  </div>
);
...
const Field = ({ label, value, icon: Icon, isLight }) => (
  <div className="group/field relative">
    ...
    <div className="relative">
      <p className={`text-sm font-semibold uppercase tracking-widest py-3 px-5 rounded-2xl group-hover:border-primary transition-all duration-500 shadow-sm border ${
        isLight 
      ...
      <div className={`absolute right-5 top-1/2 -translate-y-1/2 w-1.5 h-1.5 rounded-full group-hover:bg-primary transition-colors ${isLight ? "bg-gray-200" : "bg-white/10"}`}></div>
    </div>
  </div>
);
...
const PersonnelAuthProtocol = ({ visitor, onBack, onAction }) => {
  ...
  return (
    ...
    >
      ...
      <div className="mb-20">
        <SectionHeader title="Personnel Profile Intelligence" icon={User} isLight={isLight} />
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-8">
          <Field label="Full Name" value={visitor.name || visitor.fullName} icon={User} isLight={isLight} />
          <Field label="NIC / Passport NO" value={visitor.nic} icon={Hash} isLight={isLight} />
      ...
      <div className="mb-20">
        <SectionHeader title="Visit Protocol Dynamics" icon={Briefcase} isLight={isLight} />
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          <Field label="Proposed Visit Date" value={visitor.date || visitor.proposedVisitDate} icon={Calendar} isLight={isLight} />
          <Field label="Purpose of Visitation" value={visitor.purpose || visitor.purposeOfVisitation} icon={Info} isLight={isLight} />
          <Field label="Representing Company" value={visitor.representingCompany} icon={Briefcase} isLight={isLight} />
          <Field label="Visitor Classification" value={visitor.visitorClassification} icon={Users} isLight={isLight} />
        </div>

        {/* Infrastructure Authorization Grid */}
        <div className={`mt-16 p-4 md:p-10 border rounded-[40px] shadow-inner relative overflow-hidden transition-all duration-500 ${
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-6 gap-6">
            {(visitor.areas || visitor.selectedAreas) && (visitor.areas || visitor.selectedAreas).length > 0 ? (visitor.areas || visitor.selectedAreas).map((area, idx) => (
              <motion.div
                ...
                className={`p-6 border rounded-2xl uppercase text-[12px] font-bold tracking-widest flex flex-col items-center justify-center text-center gap-3 transition-all shadow-sm group/zone ${
                  isLight ? "bg-white border-gray-200 text-[#1A1A1A]" : "bg-black/40 border-white/10 text-white"
                }`}
              >
      ...
      {(visitor.vehicle || visitor.plateNumber) && (
        <div className="mb-20">
          <SectionHeader title="Logistics & Vehicle Registry" icon={Car} isLight={isLight} />
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            <Field label="Transport Identifier" value={visitor.vehicle || visitor.plateNumber} icon={Car} isLight={isLight} />
            <Field label="Transport Type" value={visitor.vehicleType || 'CIVILIAN'} icon={Hash} isLight={isLight} />
          </div>
        </div>
      )}
          ...
          <div className="space-y-6">
            {(visitor.members || visitor.additionalVisitors).map((member, idx) => (
              ...
              >
                ...
                <Field label={`Unit Identity`} value={member.name || member.fullName} icon={User} isLight={isLight} />
                <Field label="Auth Identifier (NIC)" value={member.nic} icon={Hash} isLight={isLight} />
                <Field label="Signal Protocol (Phone)" value={member.contact || member.phoneNumber || 'N/A'} icon={Phone} isLight={isLight} />
              </motion.div>
            ))}
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 gap-6 mt-12">
            {visitor.equipment.map((item, idx) => {
                ...
                return (
                    ...
                    >
                        <div className="md:col-span-12 lg:col-span-5"><Field label="Asset Nomenclature" value={itemName} icon={Package} isLight={isLight} /></div>
                        <div className="md:col-span-6 lg:col-span-2"><Field label="Asset Qty" value={itemQty} icon={Hash} isLight={isLight} /></div>
                        <div className="md:col-span-6 lg:col-span-5"><Field label="Intake Description" value={itemDesc} icon={Info} isLight={isLight} /></div>
                    </motion.div>
                );
            })}
          </div>
