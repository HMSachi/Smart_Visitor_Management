...
const RestrictionLevel = ({ level }) => {
  ...
  return (
    <div className={`px-4 py-1.5 rounded-full text-[9px] font-black tracking-[0.2em] uppercase border flex items-center gap-2 w-fit mx-auto ${styles[level] || styles['Level 01']}`}>
      <div className={`w-1 h-1 rounded-full ${level === 'Level 03' ? 'bg-mas-red shadow-[0_0_5px_#C8102E] animate-pulse' : 'bg-current opacity-60'}`}></div>
      {level}
    </div>
  );
};
    ...
    <div className="bg-[#121214] border border-white/5 rounded-[40px] overflow-hidden flex flex-col shadow-3xl animate-fade-in-slow relative">
      ...
      <div className="px-10 py-10 border-b border-white/5 bg-[#161618] flex flex-col xl:flex-row justify-between items-start xl:items-center gap-8 shadow-inner relative z-10">
        <div className="flex items-center gap-6">
          ...
          <div>
            <h2 className="uppercase text-white text-[11px] font-black tracking-[0.4em]">Enforcement Registry</h2>
            <p className="text-mas-text-dim uppercase text-[9px] font-black tracking-widest mt-1 opacity-40 italic">Global Restriction Database Node 08</p>
          </div>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-6 w-full xl:w-auto">
          ...
          <div className="flex gap-4">
            ...
            <button className="flex items-center justify-center gap-3 px-8 py-4 bg-mas-red text-white border border-transparent rounded-2xl uppercase text-[10px] font-black tracking-[0.3em] hover:bg-red-500 transition-all group shrink-0 shadow-[0_10px_20px_rgba(200,16,46,0.2)]">
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-[#121214] border-b border-white/5">
              <th className="px-10 py-6 text-[10px] font-black tracking-[0.3em] uppercase text-mas-text-dim opacity-40">Identity_Identifier</th>
              <th className="px-10 py-6 text-[10px] font-black tracking-[0.3em] uppercase text-mas-text-dim opacity-40">Operational_Restriction_Logic</th>
              <th className="px-10 py-6 text-[10px] font-black tracking-[0.3em] uppercase text-mas-text-dim opacity-40 text-center">Threat_Level</th>
              <th className="px-10 py-6 text-[10px] font-black tracking-[0.3em] uppercase text-mas-red text-right pr-12 italic">Control</th>
            </tr>
          </thead>
              ...
              {filtered.map((item, idx) => (
                ...
                >
                  <td className="px-10 py-8">
                    <div className="flex items-center gap-6">
                      <div className="w-14 h-14 bg-[#121214] border border-white/5 rounded-2xl flex items-center justify-center text-mas-red text-lg font-black italic group-hover:border-mas-red/50 group-hover:scale-110 transition-all duration-500 shadow-xl relative overflow-hidden">
                        <div className="absolute inset-0 bg-mas-red/5 opacity-0 group-hover:opacity-100 transition-opacity"></div>
                        {item.name.split(' ').map(n => n[0]).join('')}
                      </div>
                      <div>
                        <p className="text-white uppercase text-[12px] font-black tracking-widest mb-1.5 group-hover:text-mas-red transition-colors">{item.name}</p>
                        <p className="text-mas-text-dim/40 uppercase text-[9px] font-black tracking-[0.2em] font-mono italic">{item.docId}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-10 py-8">
                    <div className="flex flex-col gap-2">
                      <p className="text-white/80 uppercase text-[10px] font-black tracking-widest leading-relaxed max-w-md">{item.reason}</p>
                      <div className="flex items-center gap-4">
                        <p className="text-mas-text-dim/20 uppercase text-[8px] font-black tracking-[0.3em] flex items-center gap-2 italic">
                  ...
                  <td className="px-10 py-8 text-right pr-12">
                    <div className="flex justify-end gap-3">
                      <button className="w-10 h-10 rounded-xl flex items-center justify-center bg-white/[0.02] border border-white/5 text-mas-text-dim hover:text-white hover:bg-mas-red hover:border-mas-red transition-all duration-500 shadow-xl group/btn">
                        <Trash2 size={16} className="group-hover/btn:scale-110 transition-transform" />
                      </button>
                      <button className="w-10 h-10 rounded-xl flex items-center justify-center bg-white/[0.02] border border-white/5 text-mas-text-dim hover:text-white hover:bg-[#121214] hover:border-white/20 transition-all duration-500 shadow-xl group/btn">
      ...
      <div className="px-10 py-6 border-t border-white/5 bg-[#121214]/50 flex justify-between items-center relative">
        <div className="flex items-center gap-4 text-mas-red">
          ...
          <span className="text-[10px] font-black uppercase tracking-[0.3em] italic">{filtered.length} RESTRICTED_NODES_IN_CURRENT_MATRIX</span>
        </div>
        <div className="flex items-center gap-3">
          <span className="text-mas-text-dim/20 text-[9px] font-black uppercase tracking-[0.4em]">Node_Health: OPERATIONAL</span>
