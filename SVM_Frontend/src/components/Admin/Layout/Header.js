...
const Header = () => {
  ...
  const displayName = user?.ResultSet?.[0]?.VA_Name || user?.ResultSet?.[0]?.VCP_Name || 'Administrator';
  const displayEmail = user?.ResultSet?.[0]?.VA_Email || user?.ResultSet?.[0]?.VCP_Email || '';
  const role = user?.ResultSet?.[0]?.VA_Role || 'Admin';
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 sm:gap-3">
        ...
        {/* User profile */}
        <button className="flex items-center gap-2.5 group cursor-default" title={role}>
          {/* Name (desktop) */}
          ...
          {/* Avatar */}
          <div
            className="w-9 h-9 rounded-xl flex items-center justify-center text-white text-[13px] font-bold shrink-0 transition-transform group-hover:scale-105"
