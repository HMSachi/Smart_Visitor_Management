...
const SummaryItem = ({ icon: Icon, label, value }) => (
  <div className="flex items-start gap-3 p-2 rounded-lg bg-gray-50/50 border border-gray-100">
    <div className="w-7 h-7 rounded-md bg-white flex items-center justify-center shadow-sm text-primary">
      <Icon size={14} />
    </div>
    <div className="flex flex-col mt-0.5">
      <span className="text-[12px] font-normal text-gray-500 uppercase tracking-wide leading-none">{label}</span>
      <span className="text-[12px] font-normal text-[#0A1D37] tracking-wide mt-1">{value || "N/A"}</span>
    </div>
  </div>
);
    ...
    <div className="contact-theme-root flex bg-[#F8F9FA] overflow-hidden text-[#1A1A1A] h-screen w-full">
      <div className="flex-1 flex flex-col min-w-0 bg-[#F8F9FA] overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-4 custom-scrollbar">
          <div className="w-full max-w-[1400px] mx-auto animate-fade-in-slow pb-6">
            ...
            <div className="text-center mb-4">
              ...
              <h1 className="text-lg font-normal text-[#0A1D37] uppercase tracking-wide mb-1">Visit Registered Successfully</h1>
            ...
            <div className="bg-white p-4 md:p-5 rounded-[16px] shadow-[0_10px_40px_rgba(0,0,0,0.03)] border border-gray-100 space-y-4 max-w-none w-full">
              ...
              <div className="flex flex-col sm:flex-row items-center gap-4 pb-4 border-b border-gray-100">
                ...
                <div className="flex-1 text-center sm:text-left">
                  <h2 className="text-[14px] font-medium text-[#0A1D37] uppercase tracking-wide mb-0.5">{selectedVisitorDetails?.VV_Name || "Visitor"}</h2>
                  <p className="text-[12px] font-normal text-primary uppercase tracking-wide mb-2">{selectedVisitorDetails?.VV_Visitor_Type || "Guest"}</p>
              ...
              <div className="pb-4 border-b border-gray-100">
                <h3 className="text-[12px] font-medium text-[#0A1D37] uppercase tracking-wide mb-2.5 flex items-center gap-2">
                    ...
                    <div className="divide-y divide-gray-100">
                      {vehicles.filter(v => v.number).map((v, i) => (
                        <div key={i} className={`flex flex-col sm:flex-row sm:items-center justify-between gap-2 px-3 py-1.5 ${i % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}`}>
                          ...
                          <span className="text-[11px] font-medium text-[#1A1A1A] flex-1 sm:text-right">{v.number}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                ) : (
                  <p className="text-[11px] font-medium text-gray-400 italic">None registered</p>
                )}
              </div>
                    ...
                    <div className="divide-y divide-gray-100">
                      {people.filter(p => p.name).map((p, i) => (
                        <div key={i} className={`flex flex-col sm:flex-row sm:items-center justify-between gap-2 px-3 py-1.5 ${i % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}`}>
                          ...
                          <span className="text-[11px] font-medium text-gray-600 uppercase flex-1 sm:text-center">{p.nic}</span>
                          <span className="text-[11px] font-medium text-gray-600 flex-1 sm:text-right">{p.phone || "-"}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                ) : (
                  <p className="text-[11px] font-medium text-gray-400 italic">No additional people</p>
                )}
              </div>
                      ...
                      {items.filter(i => i.name).map((i, idx) => (
                        <div key={idx} className={`flex flex-col sm:flex-row sm:items-center justify-between gap-2 px-3 py-1.5 ${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}`}>
                          ...
                          <div className="w-16 flex justify-center">
                            <span className="text-[11px] font-medium text-primary bg-primary/5 px-2 py-0.5 rounded tracking-wide">x{i.quantity}</span>
                          </div>
                          <span className="text-[11px] font-medium text-gray-500 flex-[3] sm:text-right truncate">{i.description || "-"}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                ) : (
                  <p className="text-[11px] font-medium text-gray-400 italic">No items declared</p>
                )}
              </div>
            </div>
            ...
            <div className="flex flex-col md:flex-row items-center justify-center gap-3 mt-6">
              <button onClick={handleDone} className="w-full md:w-auto px-6 py-2 bg-[#C8102E] hover:bg-[#A60D26] text-white text-[12px] font-medium uppercase tracking-wide rounded-[8px] shadow-[0_5px_15px_rgba(200,16,46,0.15)] transition-all active:scale-95 flex items-center justify-center gap-2">
                Conclude Process <ArrowLeft className="rotate-180" size={14} />
              </button>
              <button onClick={() => window.print()} className="w-full md:w-auto px-6 py-2 bg-white border border-gray-200 text-[#0A1D37] text-[12px] font-medium uppercase tracking-wide rounded-[8px] hover:bg-gray-50 transition-all flex items-center justify-center gap-2">
