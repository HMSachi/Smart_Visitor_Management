...
import {
  Search, X, Calendar, MapPin, ClipboardList, 
  Send, Edit, CheckCircle2, XCircle, Clock, Hash,
  AlertCircle
} from 'lucide-react';
...
const MyRequests = () => {
  ...
  const { visitRequestsByVis, isLoading, error } = useSelector((state) => state.visitRequestsState);
  
  // Extract Visitor ID from login session
  const user = useSelector(state => state.login.user);
  const visitorId = user?.ResultSet?.[0]?.VV_Visitor_id || user?.ResultSet?.[0]?.VA_Admin_id || 4; // Mocking 4 for now
  ...
  const handleSubmit = (e) => {
    ...
    closeModal();
    // Re-fetch to see update (Redux action already does GetAll requests, but let's be sure)
    setTimeout(() => dispatch(GetVisitRequestsByVisitor(visitorId)), 1500);
  };

  const filteredRequests = visitRequestsByVis ? visitRequestsByVis.filter(req => 
    ...
    <div className="min-h-screen bg-[var(--color-bg-default)] text-white p-6 md:p-12 font-sans relative overflow-hidden">
        ...
        <div className="max-w-[1400px] mx-auto relative z-10">
            <header className="mb-12 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                <div>
                    <span className="text-primary font-black uppercase tracking-[0.4em] text-xs">Security Clearance</span>
                    <h1 className="text-4xl md:text-5xl font-black text-white mt-2 uppercase tracking-tighter">My Visit <span className="text-primary italic">Requests</span></h1>
                </div>
            ...
            <div className="bg-black/20 border border-white/5 rounded-[40px] overflow-hidden backdrop-blur-xl shadow-2xl">
                {isLoading ? (
                    <div className="p-24 flex flex-col items-center justify-center">
                        ...
                        <p className="mt-6 text-gray-400 font-bold uppercase tracking-[0.4em] text-xs">Accessing System Registry...</p>
                    </div>
                ) : error ? (
                    <div className="p-24 text-center">
                        ...
                        <p className="text-primary font-bold uppercase tracking-[0.2em]">{error}</p>
                    </div>
                ) : (
                    <TableContainer component={Paper} className="bg-transparent shadow-none border-none">
                        <Table sx={{ minWidth: 650 }}>
                            <TableHead className="bg-white/[0.02]">
                                <TableRow>
                                    <TableCell className="text-gray-500 font-black uppercase tracking-widest text-[10px] border-b-white/5 py-8 px-10">Protocol ID</TableCell>
                                    <TableCell className="text-gray-500 font-black uppercase tracking-widest text-[10px] border-b-white/5 py-8 px-10">Date</TableCell>
                                    <TableCell className="text-gray-500 font-black uppercase tracking-widest text-[10px] border-b-white/5 py-8 px-10">Destination</TableCell>
                                    <TableCell className="text-gray-500 font-black uppercase tracking-widest text-[10px] border-b-white/5 py-8 px-10">Purpose</TableCell>
                                    <TableCell className="text-gray-500 font-black uppercase tracking-widest text-[10px] border-b-white/5 py-8 px-10">Status</TableCell>
                                    <TableCell className="text-gray-500 font-black uppercase tracking-widest text-[10px] border-b-white/5 py-8 px-10" align="right">Controls</TableCell>
                                </TableRow>
                            </TableHead>
                            <TableBody>
                                {filteredRequests.length > 0 ? (
                                    filteredRequests.map((req) => (
                                        <TableRow key={req.VVR_Request_id} hover className="hover:bg-white/[0.02] transition-all">
                                            <TableCell className="px-10 py-8 border-b-white/5">
                                                <div className="flex items-center gap-3">
                                                    <div className="w-8 h-8 rounded-lg bg-primary/10 flex items-center justify-center text-primary border border-primary/20">
                                                        <Hash size={14} />
                                                    </div>
                                                    <span className="text-white font-mono tracking-tighter text-lg">#{req.VVR_Request_id}</span>
                                                </div>
                                            </TableCell>
                                            <TableCell className="px-10 py-8 border-b-white/5">
                                                <div className="flex items-center gap-3 text-gray-300">
                                                    <Calendar size={14} className="text-primary/50" />
                                                    <span className="text-[13px] font-bold tracking-widest uppercase">{req.VVR_Visit_Date ? req.VVR_Visit_Date.split('T')[0] : 'N/A'}</span>
                                                </div>
                                            </TableCell>
                                            <TableCell className="px-10 py-8 border-b-white/5">
                                                <div className="flex items-center gap-3 text-gray-300">
                                                    <MapPin size={14} className="text-primary/50" />
                                                    <span className="text-[13px] font-bold tracking-widest uppercase">{req.VVR_Places_to_Visit || '-'}</span>
                                                </div>
                                            </TableCell>
                                            <TableCell className="px-10 py-8 border-b-white/5">
                                                <p className="text-white font-medium tracking-wide text-sm opacity-80 line-clamp-1">{req.VVR_Purpose || '-'}</p>
                                            </TableCell>
                                            <TableCell className="px-10 py-8 border-b-white/5">
                                                <StatusBadge status={req.VVR_Status} />
                                            </TableCell>
                                            <TableCell className="px-10 py-8 border-b-white/5" align="right">
                                                <button 
                                    ...
                                    <TableRow>
                                        <TableCell colSpan={6} align="center" className="py-24 border-none text-gray-600 font-bold uppercase tracking-[0.3em] text-xs">
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-black/90 backdrop-blur-md animate-fade-in">
                ...
                >
                    <div className="p-10 border-b border-white/5 bg-white/[0.01] flex justify-between items-center">
                        <div>
                            <span className="text-primary font-black uppercase tracking-[0.4em] text-[10px]">Correction Protocol</span>
                            <h2 className="text-2xl font-black text-white mt-1 uppercase tracking-tight">Review & Edit <span className="text-primary italic">Request</span></h2>
                        </div>
                    ...
                    <form onSubmit={handleSubmit} className="p-10 space-y-8">
                        <div className="grid grid-cols-1 gap-8">
                             <div className="space-y-3">
                                <label className="text-[11px] font-black text-gray-500 uppercase tracking-[0.3em] ml-1">Visit Date</label>
                            ...
                            <div className="space-y-3">
                                <label className="text-[11px] font-black text-gray-500 uppercase tracking-[0.3em] ml-1">Visiting Areas</label>
                                <div className="relative group">
                            ...
                            <div className="space-y-3">
                                <label className="text-[11px] font-black text-gray-500 uppercase tracking-[0.3em] ml-1">Updated Purpose</label>
                                <div className="relative group">
                                    <ClipboardList className="absolute left-4 top-5 text-primary/40 group-focus-within:text-primary transition-colors" size={18} />
                                    <textarea 
                                        required
                                        name="VVR_Purpose"
