                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {administrators.map((admin, index) => (
                        <div key={index} className="bg-white p-4 shadow rounded border">
                            <h3 className="text-lg font-semibold">{admin.name || admin.FirstName || 'Administrator'}</h3>
                            <p className="text-gray-600">{admin.email || admin.EmailAddress || 'No Email Provided'}</p>
                        </div>
                    ))}
                </div>
            )}
