...
const Layout = ({ children }) => {
    ...
    return (
        <Box className="h-screen w-full flex bg-mas-dark-900 overflow-hidden relative">
            {isAdminPath && !isLoginPage && <Sidebar />}

            <Box
                component="main"
                className={`flex-1 flex flex-col min-w-0 transition-all duration-500 ease-in-out overflow-y-auto overflow-x-hidden
